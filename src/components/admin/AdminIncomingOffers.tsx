...
import type { AdminOffer } from "./AdminOfferCard";
...
export function AdminIncomingOffers({ propertyUuid, statusFilter, title }: Props) {
  ...
  const fetchOffers = useCallback(async () => {
    ...
    try {
      ...
      setOffers(Array.isArray(json.data) ? (json.data as AdminOffer[]) : []);
    ...
    <div>
      ...
      {empty && (
        <div className="rounded-xl border border-gray-200 bg-white p-10 text-center dark:border-gray-700 dark:bg-gray-900/40">
          <p className="font-medium text-gray-900 dark:text-white">No incoming offers yet</p>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            When tenants submit offers on your listings they will appear here.
          </p>
        </div>
      )}
              ...
              {offers.map((offer) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 align-middle">
                    <p className="max-w-[180px] truncate font-medium text-gray-900 dark:text-gray-100 text-sm">
                      {offer.property?.title || "—"}
                    </p>
                    {offer.property?.property_uuid && (
                      <p className="mt-0.5 font-mono text-[10px] text-gray-400 truncate max-w-[180px]">
                        {offer.property.property_uuid}
                      </p>
                    )}
                  </td>
