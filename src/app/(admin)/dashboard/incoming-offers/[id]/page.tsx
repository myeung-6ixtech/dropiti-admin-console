...
import { AdminOfferDetail, type AdminOffer } from "@/components/admin/AdminOfferCard";

export default function IncomingOfferDetailPage() {
  const { isLoading: authLoading, isAuthenticated } = useAuth();
  const params = useParams();
  const offerId = params?.id ? String(params.id) : null;
  ...
  const fetchOffer = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/v1/admin/offers/incoming?limit=100&offset=0`,
        { credentials: "include" }
      );
      ...
      const found = (json.data as AdminOffer[]).find(
  ...
  return (
    <div className="p-2 md:p-0">
      ...
      <div className="mb-6">
        ...
        {offer && (
          <p className="mt-1 font-mono text-xs text-gray-400">{offer.offer_key}</p>
        )}
      </div>
